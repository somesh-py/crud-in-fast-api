...
async def table(request: Request):
    ...
    return templates.TemplateResponse("table.html", {'request': request, 'persons': persons})


@router.get("/update/{id}", response_class=HTMLResponse)
async def read_item(request: Request, id: int):
    user = await User.get(id=id)
    return templates.TemplateResponse("update.html", {'request': request, 'id': user})


@router.post("/update-details/", response_class=HTMLResponse)
async def updatedetails(request: Request, Name: str = Form(...),
                        Email: str = Form(...),
                        Phone: str = Form(...),
                        Id: int = Form(...)):
...
async def delete(request: Request, id: int):
    user = await User.filter(id=id).delete()
    return RedirectResponse('/table/')
